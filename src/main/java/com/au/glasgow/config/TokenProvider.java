...
/*
from Recruitment app provided by Ram (RaFT code)
 */
@Component
public class TokenProvider implements Serializable {
     ...
     * @return
     */
    //has been altered from original copied code
    public String generateTokenFromGoogleToken(String email, String googleToken) {
        ...
        String authorities = autho.stream().map(GrantedAuthority::getAuthority).collect(Collectors.joining(","));
        return Jwts.builder().setSubject(email).setId(googleToken).claim(AUTHORITIES_KEY, authorities)
                .setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(new Date(System.currentTimeMillis() + TOKEN_VALIDITY * 1000))
                .signWith(SignatureAlgorithm.HS256, SIGNING_KEY).compact();
    }
